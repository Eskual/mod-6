import os
import shutil
from for_normalizing import normalize

def sorting(directory, main_directory):
    images = []
    videos = []
    documents = []
    music = []
    ...
    if len(os.listdir(directory)) == 0:  # Видалення пустих папок напочатку рекурсії
        print(str(directory), 'is empty! DELETING 1')
        ...
        for object in objects:  # Додавання в відповідний список об'єктів та переміщення об'єкту у відповідну папку
            ...
            if os.DirEntry.is_dir(
                    object):  # Перевірка, чи потрібно занурюватись у рекурсію (ігнорування спеціальних папок)
                ...
                os.replace(os.path.abspath(object),
                           main_directory + '\\images\\' + normalize(os.path.splitext(object.name)[0]) + (
                               os.path.splitext(object.name)[-1]))  # Переміщення у відповідну папку файла
                ...
                os.replace(os.path.abspath(object),
                           main_directory + '\\videos\\' + normalize(os.path.splitext(object.name)[0]) + (
                               os.path.splitext(object.name)[-1]))
                ...
                os.replace(os.path.abspath(object),
                           main_directory + '\\documents\\' + normalize(os.path.splitext(object.name)[0]) + (
                               os.path.splitext(object.name)[-1]))
                if os.path.splitext(object)[-1] not in known_formats:
                    known_formats.append(os.path.splitext(object)[-1])
                ...
                os.replace(os.path.abspath(object),
                           main_directory + '\\music\\' + normalize(os.path.splitext(object.name)[0]) + (
                               os.path.splitext(object.name)[-1]))
                if os.path.splitext(object)[-1] not in known_formats:
                    known_formats.append(os.path.splitext(object)[-1])
                ...
                shutil.unpack_archive(str(os.path.abspath(object)),
                                      main_directory + '\\archives\\' + str(object.name).rsplit('.', 1)[0])
        ...
        if len(links_to_folders) > 0:  # рекурсивний виклик функції для перевірки вкладених папок
            ...
            for link in links_to_folders:
                ...
                print(sorting(os.path.abspath(link), main_directory), ' ')
                if os.path.exists(link):

                    if len(os.listdir(link)) == 0:  # Видалення пустих папок на виході з рекурсії
                        print(str(link), 'is empty! DELETING 2')
                        os.rmdir(os.path.abspath(link))
                        return ''
                    os.rename(os.path.abspath(link), directory + '\\' + normalize(
                        link.name))  # перейменування папки при виході з рекурсивного занурення в папку
